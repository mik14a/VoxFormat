...
FVoxNodeTransform* FVoxNodeTransform::Read(const void*& data, size_t& size)
{
	auto* ntrn = new FVoxNodeTransform();
	auto id = ReadData<uint32_t>(data, size);
	if (FVoxNodeTransform::Tag != id) return ntrn;

	ntrn->Content = ReadData<int32_t>(data, size);
	ntrn->Children = ReadData<int32_t>(data, size);
	ntrn->Id = ReadData<int32_t>(data, size);
	...
	ntrn->Hidden = hidden != attribute.end() ? (std::stoi(hidden->second) != 0) : false;
	ntrn->Child = ReadData<int32_t>(data, size);
	ReadData<int32_t>(data, size);  // reserved
	ntrn->Layer = ReadData<int32_t>(data, size);
	auto frames = ReadData<int32_t>(data, size);
